...
def main():
    ...
    matches = re.search(r'https?://codeforces.com/(?:problemset/problem/(?P<contest_id>\d+)/(?P<problem_index>\w+)|contest/(?P<contest_id_2>\d+)/problem/(?P<problem_index_2>\w+))', cod).groupdict()
    
    contest_id = matches["contest_id"] or matches["contest_id_2"]
    problem_index = matches["problem_index"] or matches["problem_index_2"]

    submit_code(contest_id, problem_index, cod, DEFAULT_LANGUAGE_CODE)
...
function! CFLastVerdict()
    pyfile /path/to/your/plugin/checker.py
endfunc

command! CodeForceSubmit  call  CFSubmit()
